...
(() => {
  let textElm = document.querySelectorAll(".s3__list-item-price-changed");
  let btns = document.querySelectorAll(".s3__list-item-bullets-cnahged .s3__list-item-bullet-text");
  if(!btns.length){
    return false;
  }
  ...
  const f = e => {
    ...
    textElm.forEach(e => e.classList.remove("active"));
    
    if(tthis.classList.contains("s3__list-item-bullet-text30")){
      textElm[0].classList.add("active");
    } 
    
    if(tthis.classList.contains("s3__list-item-bullet-text90")){
      textElm[1].classList.add("active");
    } 
  }

  btns.forEach(e => e.addEventListener("click", f));
