  ...
  forms.forEach((form) => {
    form.addEventListener("submit", function (event) {
      ...
      event.preventDefault();
        ...
        //   finish Btn
      let finishBtn = this.querySelector(".main-form__btn");
      ...
      if (isNameValidFlag && isPhoneValidFlag) {
        ...
        xhr.open('POST', 'https://slivkin-fulfillment.ru/wp-admin/admin-ajax.php?action=callback_mail');
        xhr.send(formData);
...
// load map
let mapWeb = document.querySelector(".map-web");
let mapMob = document.querySelector(".map-phone");
if (window.innerWidth < 1000) {
    mapWeb.style.display = "none";
    mapMeb.style.display = "block";
} else {
    mapWeb.style.display = "block";
    mapMeb.style.display = "none";
}